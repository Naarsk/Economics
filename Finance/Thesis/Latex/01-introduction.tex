...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Trends in private equity capital and dry powder levels over time. Data from Preqin.}
    \label{fig:pe_trends}
\end{figure}

Unlike public equities, where investor beliefs can be inferred directly from market prices, private equity beliefs are not observable in real time.
...
These communications often combine hard information with softer language that can influence perceptions.
It remains unclear whether LPs are consistently able to extract the predictive content of these narratives or if they are sometimes influenced by sentiment and bias.

Historical episodes illustrate the stakes.
During the global financial crisis (2008–2009), many PE funds were slow to revise net asset values (NAVs), creating a disconnect between reported valuations and underlying fundamentals.
...
\begin{figure}
    ...
    \caption{Private equity returns (source: Preqin) are calculated using the NAV reported by GPs. Therefore, they tend to be much less volatile than market returns.}
    \label{fig:pe_vs_pm}
\end{figure}
...
The aim of this thesis is to introduce a methodology to use LLMs to process GPs report and extract information effectively and quantify the sentiment expressed in these documents.

\section{Literature Review}\label{sec:literature-review}

The starting point for this thesis is the literature on belief formation in asset pricing.
...
Bybee~\citep{bybee2025ghost} introduces a framework in which belief formation is shaped by language patterns.
...
\begin{quote}
    \emph{How are beliefs about private equity performance formed from qualitative
    information, and do large language models exhibit the same biases as human
    investors when generating such beliefs?}
\end{quote}

To address this question, I leverage a dataset of over 20,000 PE investment reports from Preqin, enriched with cash flow data on NAVs,
...
\begin{enumerate}
    ...
    text.
    \item \textbf{Bias in Reporting and Interpretation:} I test whether
    GP-reported NAVs and qualitative narratives are biased in a predictable
    way and examine whether LLMs replicate, amplify, or correct such biases.
...
\section{Thesis Roadmap}\label{sec:thesis-roadmap}

The remainder of the thesis is structured as follows.
Chapter~2 reviews the literature on belief formation in financial markets,
the use of LLMs in finance, and the unique features of private equity.
Chapter~3 develops the theoretical framework, building on Bybee’s
\emph{Ghost in the Machine} model and adapting it to the context of
illiquid assets. Chapter~4 describes the data, including the
investment reports, cash flows, and preprocessing steps. Chapter~5
details the methodology, covering LLM selection, prompt design, and
validation metrics. Chapter~6 explains the empirical implementation,
including model training, backtesting, and sensitivity analyses.
Chapter~7 presents the results, while Chapter~8 discusses their
implications for LPs, GPs, and risk management. Finally, Chapter~9
concludes and outlines avenues for future research.

By connecting recent advances in language modeling with fundamental
questions about belief formation, this thesis contributes to our
understanding of how information shapes expectations in one of the
most opaque segments of capital markets. The results have implications
not only for academic debates but also for practitioners seeking to
navigate an increasingly complex and data-rich investment landscape.
